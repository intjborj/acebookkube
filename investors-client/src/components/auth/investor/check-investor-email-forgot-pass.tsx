...
import VerifyEmailCodeFP from './verify-email-code-forgot-pass';
...
const InvEmailCheckingFPView = ({ idNumber= "none" }: Props) => {
        ...
        sendVerif({
            ...
            .then((resp) => {
                ...
                if (_.get(resp, "data.verifyInvEmail.data.accepted").length >= 1) {
                    setState((p) => ({ ...p, codeSent: true, saveEmail: sendEmail, userId: _.get(resp, "data.verifyInvEmail.data._id") }))
                }
                ...
                <>
                    {
                        // _.get(specInv, "specificInvestor.data._id") ?
                            <>
                                {state.codeSent == true && state.userId ?
                                    <VerifyEmailCodeFP
...
export default InvEmailCheckingFPView